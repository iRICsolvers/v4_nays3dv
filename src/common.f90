...
!------------------------------------------------
  subroutine initial
    ...
    eave_up = eave_up/float(ny) !初期上流平均河床
    eave_dw = eave_dw/float(ny) !初期下流平均河床
    have_up_ini = have_up_ini/float(ny) !初期上流平均水位
    have_dw_ini = have_dw_ini/float(ny) !初期下流平均水位
    hsave_up_ini = have_up_ini - eave_up  !初期上流平均水深
    hsave_dw_ini = have_dw_ini - eave_dw  !初期下流平均水深
