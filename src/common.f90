...
!-----------------------------------------
      ...
      real(8),dimension(:,:,:),allocatable::u38,v38,w38,p38,c38,sigma38,&
        ob38,q38,snu_t38
...
!       write(44,'(2i5,4f10.3)') &
!        i,j,z(i,j),h_node(i,j),h_node(i,j)-z(i,j),dep_min
        if(j_mindep==1.and.(z(i,j).gt.h_node(i,j)-dep_min)) then
         z(i,j)=h_node(i,j)-dep_min
         z8(i+1,j+1)=z(i,j)
!        write(44,'(a1,2i5,f10.3)') 'D',i,j,z(i,j)
        end if
       end do
      end do
! z,h_node ---> node
...
!     do k=0,nz+1
!      write(44,*) k,dz(k),xi(k),dzk(k),xxi(k)
!     end do
...
!     open(42,file='c:\tmp\obst3d.d',status='unknown')
!     do i=0,nx+1
!      write(42,'(a2,i2)') 'i=',i
!      do k=0,nz+1
!       write(42,'(i3,35i1)') k,(obst3d(i,j,k),j=1,ny)
!      end do
!     end do
!     close(42)
!     stop
...
!     do i=0,nx
!      write(44,'(i5,6f8.4)') i,x(i,0),y(i,0),x(i,ny),y(i,ny),x(i,nym),y(i,nym)
!     end do

!     do i=0,nx
!      write(44,'(i5,3f8.4)') i,ds(i,0),ds(i,nym),ds(i,ny)
!      write(44,'(i5,3f8.4)') i,dn(i,1),dn(i,nym),dn(i,ny)
!     end do
!
! xds2,yds2‚ÌŒvZ
!
...
!     do i=1,nx-1
!      write(44,'(i5,4f10.5)') i,xds2(i,0),xds2(i,ny),yds2(i,0),yds2(i,ny)
!     end do
!
...
!     do i=1,nx-1
!      write(44,'(i5,4f10.5)') i,curv(i,0),curv(i,ny),radi(i,0),radi(i,ny)
!     end do
!     stop

!     do i=0,nx
!      write(44,'(i5,3f10.5)') i,h(i,1),h(i,nym),h(i,ny)
!     end do
!     stop
...
!     write(44,*)'A',hs_up,sb,u_ave
!     write(44,'(12f8.4)') (u00(k),k=0,nz+1)
!
! cd0 –€CŒW”‚ÌŒˆ’è
!
      inum=0
      ...
      do i=1,nx
       do j=1,ny
        do k=1,nz
         ...
         ycn(i,j,k)=yc(i,j,k)
!        write(44,'(3i4,f10.6)') i,j,k,ycn(i,j,k)
        end do
       end do
      end do
! 
