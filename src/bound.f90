...
contains
	...
	!*******************************
		...
		real(8)::dwds,dhds,dwdxi,duds,dd
		real(8)::dxii
		...
		do i=0,nx
			do j=1,ny
				...
				if(j_surf==0.or.(j_surf==1.and.time<stime_surf)) then
					...
					dwdxi=(w(i+1,j,nz)+w(i,j,nz)-w(i+1,j,nz-1)-w(i,j,nz-1)) /(dz(nz)*2.)
					if(i==0) then
		...
		do j=0,ny+1
			do k=0,nz+1
				if(j_west==1) then !West Closed
					u(0,j,k)=0.
					u(-1,j,k)=0.
				else if(j_west==2) then !West Open ăŹJú
					if(j_qin==0) then !ăŹÍt[
						u(-1,j,k)=u(0,j,k)
					else if(j_qin>=1) then !ŹĘđ^Śé-->ŹŹđ^Śé
						if(obst(1,j)==1) then
							u(-1,j,k)=0.
							u(0,j,k)=0.
						else
				...
				! if(u(nx,j,k)<0) u(nx,j,k)=0. !şŹĹu<0ÍFßČ˘
					u(nx+1,j,k)=u(nx,j,k)
				end if
			end do
		end do
		...
		do i=1,nx
			do j=0,ny
				...
				if(j_surf==0.or.(j_surf==1.and.time<stime_surf)) then
					...
					dwdxi=(w(i,j+1,nz)+w(i,j,nz)-w(i,j+1,nz-1)-w(i,j,nz-1))/(dz(nz)*2.)
					if(j==0) then
